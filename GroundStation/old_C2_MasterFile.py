...
filenameBase=str(datetime.datetime.fromtimestamp(datetime.datetime.now(tz=pytz.utc).timestamp()).strftime('%Y-%m-%d %H:%M:%S'))

historicalPositionsSaved = 10 #number of breadcrumbs to be saved

#legacy variables that do not have an inpact on the system functionality. Will need to be removed by changing some file parameters
radar_array = []
array_new = -1
array_old = 0
status = 1
...
#Thread Class that dictates the Situational Awareness Map Refreash
...
#Thread that dictates the rate at which C2 recieves messages from the Ground Camera
...
MsgHeadrC=int("1e91",16) # randomly chosen for experiment, system defined message header for Ground Camera Control messages
...
MsgHeadrD=int("a33e",16) # randomly chosen for experiment, system defined message header for Ground Camera Control messages
...
def main():
    ...
    while True:
        ...
        program.update_log(podDictionary, TgtEstimatorDict, trueUAVDictionary, radar_Dictionary_type48, radar_Dictionary_type34, discoveryDroneDict, trueUAVDictionary, TgtEstimatorDict,
                           predictedPositions_Dictionary, DiscoveryDroneOffset_dict) #sending the dictionary infromation found below
...
def readDiscoverDroneMsg():
    ...
    ##Image Recieve code from discovery Drone
        ...
        # get metadata
            ...
            #cv2.imwrite('DiscoveryDrone_' + str(int(datetime.datetime.now(tz=pytz.utc).timestamp())) + '_image_' + str(imageCtr) + '.jpg', frame)
            ...
            if (GUI_Master.follow_me_flag==1): #Press P on the keybaord to enact automatic tracking
                #cv2.destroyAllWindows()
                RadarFollow = True
                print(RadarFollow)
...
def readGndCamMsg(imageCtr):
        try:
            ...
            if (recvHdrG == MsgHeadrG): #confirm header matches expected message type
                ...
                #cv2.imwrite('ts_' + str(int(datetime.datetime.now(tz=pytz.utc).timestamp())) + '_image_' + str(imageCtr) + '.jpg', frameG)
                #press 'g' in order to close the openCV windows
                if (cv2.waitKey(1) & 0xFF == ord('q')):
                    cv2.destroyAllWindows()
...
def jogGndCamera():
    ...
    try:
        ...
        messageC = [MsgHeadrC]
        messageC.append(np.uint16(68))  # metaDataC size
    ...
    #Appending the message with control information
        ...
        messageC.append(0)
        messageC.append(GUI_Master.gnd_direction) #Facing of the Ground Camera (Cardinal Direction) (0, North; 1, East; 2, South; 3, West)
...
def DiscoveryDroneControl():
    ...
    try:
        messageD = [MsgHeadrD]
        messageD.append(np.uint16(44))  # metaDataC size
        messageD.append(datetime.datetime.now(tz=pytz.utc).timestamp())
        messageD.append(0)
        messageD.append(GUI_Master.yaw)
        messageD.append(GUI_Master.drone_img_processing)
        messageD.append(GUI_Master.drone_pic_cont)
        messageD.append(0)
        messageD.append(0)
        messageD.append(0)
...
class Radar_Asterix_48: #class to extract select informatin from the ASTERIX 48 message
    def __init__(self, r_serial, radar_lat, radar_long, radar_alt, radar_velx, radar_vely, radar_velz, num_track): #need to add a parameter for the total number of tracks
...
def parse(packet, packet_type):
    ...
    if packet_type =='Simulated Acoustic Health Message':
        ...
        with open('Simulated Pod Health TST.csv', 'a', newline='') as file:
            ...
            writer.writeheader()
            writer.writerow({'TIMESTAMP': datetime.datetime.fromtimestamp(int(podDictionary[new_acoustic.pod_id].time_stamp)).strftime('%Y-%m-%d %H:%M:%S'),
                             'POD ID' : podDictionary[new_acoustic.pod_id].pod_id,
                             'LATITUDE': podDictionary[new_acoustic.pod_id].pod_lat,'LONGITUDE': podDictionary[new_acoustic.pod_id].pod_long,
                             'ALTITUDE': podDictionary[new_acoustic.pod_id].pod_alt, 'POD HEALTH' :podDictionary[new_acoustic.pod_id].pod_health,
        ...
        # #file.close()
         ...
         with open('Simulated Pod Target Messages 2TST.csv', 'a', newline='') as file:
             fieldnames = ['TIMESTAMP', 'POD ID','LATITUDE', 'LONGITUDE', 'ALTITUDE', 'POD HEALTH', 'TIME LAST MESSAGE', 'TIME LAST TARGET MESSAGE', 'TARGET CLASS', 'TARGET AOA', 'TARGET ACTIVE']
             writer = csv.DictWriter(file, fieldnames=fieldnames)
             writer.writeheader()
         ...
         TgtEstimatorDict[new_acoustic_target.track_id].time_last_message = datetime.datetime.now(tz=pytz.utc).timestamp()
         ...
         if (len(TgtEstimatorDict[new_acoustic_target.track_id].tgt_posBuffer) < historicalPositionsSaved):
             ...
             temp_buf = TgtEstimatorDict[new_acoustic_target.track_id].tgt_posBuffer[1:]  # contains the first item for the trail
             temp_buf.append(temp_pos)  # adds new position information
             TgtEstimatorDict[new_acoustic_target.track_id].tgt_posBuffer = temp_buf  # overwrites buffer and copies over latest value

         # Beginning of Target Pod Estimator log
         with open('PodEstimator2TgtTST.csv', 'a', newline='') as file:
             fieldnames = ['ESTTGTLAT', 'ESTTGTLONG', 'ESTTGTALT', 'PODID', 'POD1AOA','PODID', 'POD2AOA', 'PODID', 'POD3AOA', 'TIMESTAMP']
             writer = csv.DictWriter(file, fieldnames=fieldnames)
             writer.writeheader()
             writer.writerow({'ESTTGTLAT': TgtEstimatorDict[new_acoustic_target.track_id].est_tgt_lat, 'ESTTGTLONG' :TgtEstimatorDict[new_acoustic_target.track_id].est_tgt_long,
                              'ESTTGTALT' : TgtEstimatorDict[new_acoustic_target.track_id].est_tgt_alt,
                              'PODID':TgtEstimatorDict[new_acoustic_target.track_id].pod1_id, 'POD1AOA' : TgtEstimatorDict[new_acoustic_target.track_id].pod1_aoa,
                              'PODID' : TgtEstimatorDict[new_acoustic_target.track_id].pod2_id, 'POD2AOA': TgtEstimatorDict[new_acoustic_target.track_id].pod2_aoa,
                              'PODID' : TgtEstimatorDict[new_acoustic_target.track_id].pod3_id, 'POD3AOA' : TgtEstimatorDict[new_acoustic_target.track_id].pod3_aoa,
                 'TIMESTAMP': datetime.datetime.fromtimestamp(int(TgtEstimatorDict[new_acoustic_target.track_id].time_last_message)).strftime('%Y-%m-%d %H:%M:%S')})
         ...
         ## Step 1 Unit conversion to UTM followed by setting reference
             try:
                 ...
                 AcousticTargetUTMx, AcousticTargetUTMy = conversion(TgtEstimatorDict[new_acoustic_target.track_id].est_tgt_long, TgtEstimatorDict[new_acoustic_target.track_id].est_tgt_lat)
                 ...
                 if (len(TgtEstimatorDict) > 0):
                    ...
                    # global GroundStationUTMx, GroundStationUTMy, GroundStationUTMz
                     ...
                     AcousticTgtAlt = float(TgtEstimatorDict[new_acoustic_target.track_id].est_tgt_alt) - float(GroundStationUTMz)
                 ...
                 global filter_exists
                 if filter_exists == False: #Sensor Fusion Update for Acoustic Pod Measurement
                    global filter_id
                 ...
                 #Begin reconversion
                    ...
                    #     DiscoveryDroneOffsetlat = predictedLat + (offset/111111) #subtracting 10 meter from the predicted position
                    ...
                    #     DiscoveryDroneOffsetlong = predictedLong + (offset/(111111*np.cos(DiscoveryDroneOffsetlat)))
                    #     DiscoveryDroneOffsetalt = predictedUTMZ + 3
                    if (LattimeOffset[0] != None) & (LongtimeOffset[0] != None) & (AlttimeOffset[0] != None):
        ...
        for i in range(packet[2]):
            ...
            new_UAV_position = True_UAV(packet[0], packet[1], packet[2], packet[6*i+3], packet[6*i+4], packet[6*i+5], packet[6*i+6], packet[6*i+7], packet[6*i+8])
        ...
        #for t in range(packet[2]):
            ...
            temp_pos = True_UAV_Pos_Hist(trueUAVDictionary[new_UAV_position.tgt_index].timestamp, trueUAVDictionary[new_UAV_position.tgt_index].UAV_lat,
            ...
            if(len(trueUAVDictionary[new_UAV_position.tgt_index].posBuffer)<historicalPositionsSaved):
                ...
                temp_buf = trueUAVDictionary[new_UAV_position.tgt_index].posBuffer[1:] #contains the first item for the trail
                temp_buf.append(temp_pos) #adds new position information
                trueUAVDictionary[new_UAV_position.tgt_index].posBuffer = temp_buf #overwrites buffer and copies over latest value
        #true_UAV_pos.append(new_UAV_position)
        #print(datetime.datetime.fromtimestamp(int(trueUAVDictionary[new_UAV_position.tgt_index].timestamp)).strftime('%Y-%m-%d %H:%M:%S')) #conversion from computer timestamp to readable format
        ...
        #Beginning of True UAV log
            with open('TrueUAVInfo2TSTNew.csv', 'a', newline ='') as file:
                ...
                writer = csv.DictWriter(file, fieldnames = fieldnames)
                writer.writeheader()
                writer.writerow({'UAVID': new_UAV_position.tgt_index, 'TIMESTAMP': datetime.datetime.fromtimestamp(int(trueUAVDictionary[new_UAV_position.tgt_index].timestamp)).strftime('%Y-%m-%d %H:%M:%S'),
                    'LATITUDE': trueUAVDictionary[new_UAV_position.tgt_index].UAV_lat, 'LONGITUDE': trueUAVDictionary[new_UAV_position.tgt_index].UAV_long})
        #for i in range (len(trueUAVDictionary)):


    elif packet_type == 'Radar Type 34': #parsing of ASTERIX 34 message and population of Dictionary with select information
    ...
    #     radar_array_type34.append(radar_data)
        ...
        with open('Asterix34_TST.csv', 'a', newline='') as file:
            try:
                ...
                long_34 = int.from_bytes(radar_Dictionary_type34[new_type_34.track_id].source_long, "big", signed = False) #Longitude Unpack
                correct_long_34 = (long_34*(180/(2**23))) -360
                lat_34 = int.from_bytes(radar_Dictionary_type34[new_type_34.track_id].source_lat, "big", signed = False) #Latitude Unpack
                correct_lat_34 = lat_34 * (180 / (2 ** 23))
                alt_type ='>h' #the altitude position is reported as a 16 bit short

                alt_34 = struct.unpack(alt_type, radar_Dictionary_type34[new_type_34.track_id].source_alt) #unpacking the altitude of the radar
                global correct_alt_34
                correct_alt_34 = alt_34[0]
                writer = csv.DictWriter(file, fieldnames=fieldnames)
                writer.writeheader()
                writer.writerow({'TIME DAY': radar_Dictionary_type34[new_type_34.track_id].time_day, 'ROTATION PERIOD': radar_Dictionary_type34[new_type_34.track_id].rot_period,
                                'SOURCE ALTITUDE': radar_Dictionary_type34[new_type_34.track_id].source_alt, 'SOURCE LATITUDE' : correct_lat_34,
        ...
        # using pyproj library for cartographers in order to convert to UTM. Independently verified by Matlab program deg2utm.m
        #global conversion
        conversion = Proj("+proj= utm +zone = 13s, +north +ellps=WGS84 +datum=WGS84 +units=m +no_defs")
        ...
        AsterixLength = [2, 3, 1, 4, 2, 2, 1, 0, 3, 6, 1, 2, 4, 4, 1, 0, 4, 1, 2, 4, 2, 1, 2, 0, 7, 1, 2, 1, 2, 76, 1, 0]
        ...
        while messageEnd == False:
            ...
            while n != 8:
                if (bits[n] & ones[n]) == 1:
                    ...
                    n+= 1
                    count += 1 #the count needs to increase whether the and is successful or not, this is because a count that only increments on successful ANDs will not account for logic that returns a zero
            #messageEnd = True #for debugginf
        ...
        if asterix_type == 'Send Track':
            ...
            global new_type_48
            new_type_48 = Radar_Asterix_48(radar_data_48[38:40], radar_data_48[40:44], radar_data_48[44:48], radar_data_48[76:80], radar_data_48[70:72], radar_data_48[72:74], radar_data_48[74:76], 1) #need to add byte area for the number of tracks
            # if len(radar_data_48) > 200:
            #     second_new_type_48 = new_type_48 = Radar_Asterix_48(radar_data_48[38:40], radar_data_48[147:151], radar_data_48[151:155], radar_data_48[183:185], radar_data_48[185:187], radar_data_48[187:189], radar_data_48[189:191], 2)
            ...
            radar_Dictionary_type48[new_type_48.num_track].last_radar_message = datetime.datetime.now(tz=pytz.utc).timestamp() #for the purpose of testing, will hopefully be removed

            #radar_Dictionary_type48[new_type_48.num_track].end_of_track = plot_status

            ##Second Target Radar Information
            if len(radar_data_48) > 200:
                new_type_48 = new_type_48 = Radar_Asterix_48(radar_data_48[145:147], radar_data_48[147:151], radar_data_48[151:155], radar_data_48[183:185], radar_data_48[185:187], radar_data_48[187:189], radar_data_48[189:191], 2)
                if not new_type_48.num_track in radar_Dictionary_type48:
                    radar_Dictionary_type48[new_type_48.num_track] = Radar_Asterix_48_Track()
            ...
            with open('Asterix48_TST.csv', 'a', newline='') as file:
                try:
                    fieldnames = ['TARGET ID', 'TARGET LATITUDE', 'TARGET LONGITUDE', 'TARGET ALTITUDE', 'TARGET X VELOCITY', 'TARGET Y VELOCITY', 'TARGET Z VELOCITY']
                    ...
                    for rr in radar_Dictionary_type48:
                        ...
                        target_longitude = struct.unpack(form, radar_Dictionary_type48[rr].radar_long)  # unpacks the bytes into an integer value
                        target_long_correct = target_longitude[0] / (1e5)
                        # print(long_correct)
                        target_latitude = struct.unpack(form, radar_Dictionary_type48[rr].radar_lat)  # unpacks the bytes into an integer value
                        ...
                        #if (target_lat_correct - discoveryDroneDict[0].disc_lat/1e7 > 5/111111): #if the measured latitude is greater than 5 meters from the Discovery Drone location this is target
                        ...
                        writer.writerow({'TARGET ID' : rr, 'TARGET LATITUDE': target_lat_correct,
                                        'TARGET LONGITUDE': target_long_correct, 'TARGET ALTITUDE': target_alt_correct,
                                         'TARGET X VELOCITY': target_xvel_correct,'TARGET Y VELOCITY': target_yvel_correct,
            ...
            try:
                ...
                current_time_sensor = datetime.datetime.now(tz=pytz.utc).timestamp()
                radar_est = (current_time_sensor, TgtDistX, TgtDisty, TgtAlt, target_xvel_correct, target_yvel_correct, target_zvel_correct)
                #print(radar_est[0])
                ...
                if filter_exists is False:
                    ...
                    with open('SensorFusion_12April.csv', 'a', newline='') as file:
                        ...
                        writer = csv.DictWriter(file, fieldnames=fieldnames)
                        writer.writeheader()
                        writer.writerow({'TIMESTAMP': datetime.datetime.fromtimestamp(int(predictedPositions_Dictionary[filter_id].sensorTime)).strftime('%Y-%m-%d %H:%M:%S'),
    ...
    #Populating the Discovery Drone Dictionary
        ...
        new_DiscDrone_info = Discovery_Drone_Information(packet[0], packet[1], packet[2], packet[3], packet[4], packet[5], packet[6], packet[7], packet[8], packet[9],
                                                         packet[10], packet[11], packet[12], packet[13], packet[14], packet[15], packet[16], packet[17], 1)
        ...
        with open('DiscoveryDrone TST.csv', 'a', newline='') as file:
            fieldnames = ['TIMESTAMP', 'LATITUDE', 'LONGITUDE', 'ALTITUDE', 'X Velocity', 'Y Velocity', 'Z Velocity', 'HEADING']
            writer = csv.DictWriter(file, fieldnames=fieldnames)
            writer.writeheader()
            writer.writerow({'TIMESTAMP': datetime.datetime.fromtimestamp(int(discoveryDroneDict[new_DiscDrone_info.track_id].timestamp)).strftime('%Y-%m-%d %H:%M:%S'),
                             'LATITUDE': discoveryDroneDict[new_DiscDrone_info.track_id].disc_lat,'LONGITUDE': discoveryDroneDict[new_DiscDrone_info.track_id].disc_long,
                             'ALTITUDE': discoveryDroneDict[new_DiscDrone_info.track_id].disc_alt,'X Velocity': discoveryDroneDict[new_DiscDrone_info.track_id].vx,
                             'Y Velocity': discoveryDroneDict[new_DiscDrone_info.track_id].vy,'Z Velocity': discoveryDroneDict[new_DiscDrone_info.track_id].vz,
...
def start_server():
    ...
    """
    global status
    #receiving messages from the Simulated RADAR system and detemining their type based on the message header
    try:
    ...
    if (raw_data != 0):  # if successful capture
        ...
        if (int(raw_data[0]) == 232) & (int(raw_data[1]) == 161): #this is a simulated pod message, byte 1 = 0xe8, byte 2 = 0xa1
        ...
        elif (int(raw_data[0]) == 48) & (int(raw_data[1]) == 0): #Asterix type 48 message, First byte = 48, second byte = 0
            ...
            form = '<3H'
            for t in range(numTgts[0]): #determine the number of targets and parse message based on flexible message size
                form = form + '2H4d'
            data = struct.unpack(form, raw_data)
...
def returnToLaunch(linkup):  # Return to home Discovery Drone functon
    ...
    msgConfirm=int("d478",16) #based on message specification
    msgFormat='<2H'
    message=[msgHdr]
...
def RadarFilterPropagation(): #function to propagate estimate in time even when there is not a Radar Measurement present
    global filter_exists
    ...
    try:
        if filter_exists == True:
            ...
            predicted_state = Sensor.update_with_radar_position_estimate(target_filter, sensor_est, False) #Measurement Flag set to false
...
def sendSensorFusionWaypoint(filter_number):
    ...
    if RadarFollow == True:
        ...
        if GUI_Master.followme_select == 1: #follow me mode selection for True UAV
            goToWaypoint(senderD, trueUAVDictionary[True_UAV_Index].UAV_lat - (10/111111), trueUAVDictionary[True_UAV_Index].UAV_lat,
        ...
        if GUI_Master.followme_select == 3: #follow me mode using Sensor Fusion Estimate
            goToWaypoint(senderD, predictedPositions_Dictionary[filter_number].lat_est - (10/111111), predictedPositions_Dictionary[filter_number].long_est,
