...
filenameBase=str(int(datetime.datetime.now(tz=pytz.utc).timestamp()))
...
#  time.sleep(.5)   # simulate time between events
    try:
      frame = cap.read()
      imageCtr += 1
      cv2.imwrite('ts_'+filenameBase+'_image_'+str(imageCtr)+'.jpg',frame)
...
#      cv2.imshow("frame", frame)
      time.sleep(1) # allowed time for image processing
