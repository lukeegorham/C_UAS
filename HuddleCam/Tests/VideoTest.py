...
cap = VideoCapture(0)
noNewFile=True
msgFormat='<HHddddddddHdddd'
MsgHeadr=int("a22e",16)
blueTargets = None
...
def SndImg():
    #try:
      frame = cap.read()
      cv2.imwrite('GroundCamera_' + str(int(datetime.datetime.now(tz=pytz.utc).timestamp())) + '_image_' + '.jpg', frame)
      ...
      if(len(cnts) > 0):
         ...
         cv2.imwrite('ProcGroundCamera_' + str(int(datetime.datetime.now(tz=pytz.utc).timestamp())) + '_image_' + '.jpg', blueTargets)
      ...
      metaBase=[MsgHeadr]
      metaBase.append(np.uint16(102)) # metadata size
