...
cmd = [129, 1, 4, 53, 2, 255] #Set outdoor mode on camera
cam.open
cam.write(cmd)
cam.close
...
#Setting globals and default values

ZoomValue = 0

global latitude_deg
latitude_deg=39.009170
global longitude_deg
longitude_deg=-104.878982
...
class SndImgC2(Thread): #Initializing a thread
...
class CtrlC2(Thread): #Initializing a thread
  def __init__(self, event):
    Thread.__init__(self)
    self.stopped = event
...
def CameraAngleUtm (tx, ty, telv, cx, cy ,celv, cdirection): #Math using camera position and enemy position, with camera direction
    xdist = tx-cx
    ydist = ty-cy
    zdist = telv-celv
...
def Ctrl():
    ...
    if(recvHdr==MsgHeadr):
        ...
        #if(metadata[21] == 1):
        #        camx = int(39.009170 * 1e7)
        #        camy = int(-104.878982 * 1e7)
        #        camz = int(2163 * 10)
        #else:
