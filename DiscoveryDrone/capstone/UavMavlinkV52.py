    ...
    while True:
        try:
            ...
            if(msgId==fly2Wp_Hdr and allowGsControl):
                mode_id=mavConnection.mode_mapping()['GUIDED'] # added by Max after 16 Mar test flight, because drone was not entering guided mode
                ...
                Fly2Wp(data[3]/1e7,data[4]/1e7,data[5]/1e3,1)
...
#        mode_id=4 #mode GUIDED
        ...
        print(f"Lat: {latDeg}, Lon: {lonDeg}, Alt: {alt_mMSL}")
        #mavConnection.mav.MAV_CMD_NAV_LOITER_UNLIM
        try: 
            dl.goto_point(droneobject, latDeg, lonDeg, speed, alt_mMSL)
        # mavConnection.mav.set_position_target_global_int_send( ### MAKE ALL CAPS NEXT TIME
        #     #mavConnection.mav.SET_POSITION_TARGET_GLOBAL_INT(
        #     0,  # system time in ms
        ...
        print(f"ERROR: NC={normalControl}")
...
def InitMavlink(verbose):
    ...
    for p in ports:
        ...
        if ('fmu' in p.description or 'CubeBlack' in p.description
            ...
            serial_port = s.name
            droneobject = connect(serial_port, wait_ready=True, baud=115200)
            print(f"This is serial: {serial_port}")
